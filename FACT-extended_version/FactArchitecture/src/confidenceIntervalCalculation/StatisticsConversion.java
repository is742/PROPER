...
/**
 * This class calculates the p value from a Z score. This is done by an
 * "Error Function".
 */
public class StatisticsConversion {

	// Static normal distribution dist in order to transform z-socres into
	// probabilities.
	...
	/**
	 * The quantile calcuation is done here. -1.96 : +1.96 returns 0.95
	 * 
	 * @return double a p value
	 */
	...
	/**
	 * The zscore calcuation is done here. Example: From 0.95 returns 1.64 While
	 * 0.05 returns -1.64 And 0.975 returns 1.96
	 * 
	 * @param aProbability
	 ...
	 * 
	 * @param aQuantile
	 *            the quantile to retrieve. 0.05 returns -1.96 while 0.95
	 *            returns 1.96.
	...
	private static double calculateZscore(double aQuantile) {
		...
		if (Math.abs(q) <= split1) {
			r = const1 - q * q;
			ret = q * (((a[3] * r + a[2]) * r + a[1]) * r + a[0]) / (((b[3] * r + b[2]) * r + b[1]) * r + one);
		...
		if (r <= split2) {
			r = r - const2;
			ret = (((c[3] * r + c[2]) * r + c[1]) * r + c[0]) / ((d[2] * r + d[1]) * r + one);
		} else {
			r = r - split2;
			ret = (((e[3] * r + e[2]) * r + e[1]) * r + e[0]) / ((f[2] * r + f[1]) * r + one);
		}
