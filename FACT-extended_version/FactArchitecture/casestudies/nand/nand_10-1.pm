...
const int N = 2;//10; // number of inputs in each bundle
const int K = 1;//1; // number of restorative stages
...
const double perr1; //(0.02) probability nand works correctly
...
module multiplex
	...
	// use nand gate
	[] s=3 & z<N & c<N -> (1-perr1) : (z'=z+(1-x*y)) & (s'=0) & (c'=c+1) & (x'=0) & (y'=0) // not faulty
	         + perr1    : (z'=z+(x*y))    & (s'=0) & (c'=c+1) & (x'=0) & (y'=0); // von neumann fault
	// [] s=3 & z<N -> (1-perr1) : (z'=z+(1-x*y)) & (s'=0) & (c'=c+1) & (x'=0) & (y'=0) // not faulty
	//         + perr1    : (z'=z+(x*y))    & (s'=0) & (c'=c+1) & (x'=0) & (y'=0); // von neumann fault
